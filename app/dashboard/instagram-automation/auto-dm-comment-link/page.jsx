...
export default function AutoDmCommentLinkPage() {
  ...
  const handleDelete = async (mediaId) => {
    setDeleteId(mediaId);
    try {
      const commentCheckResponse = await fetch(
        `/api/media/comments-count?mediaId=${mediaId}`
      );
      if (!commentCheckResponse.ok) {
        throw new Error("Failed to check for related comments");
      }
      const data = await commentCheckResponse.json();
      setCommentData(data);
    } catch (err) {
      setError("Error checking comments: " + err.message);
      toast.error("Error checking comments: " + err.message, {
        position: "top-center",
      });
      return;
    }
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(`/api/media/settings?mediaId=${deleteId}`, {
        method: "DELETE",
      });
      ...
      setMediaList((prev) =>
        prev.filter((media) => media.mediaId !== deleteId)
      );
      toast.success(
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      ...
      setIsLoading(true);
      fetchMediaSettings().finally(() => setIsLoading(false));
    }
  }, [status, router]);
  ...
  const fetchMediaSettings = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch media settings");
      }
  ...
  const handleEdit = (media) => {
    router.push(`/dashboard/auto-dm-comment-link/automation/${media.mediaId}`);
  };
  ...
  //       throw new Error("Failed to check for related comments");
  //     }
  //     const commentData = await commentCheckResponse.json();

  //     if (commentData.hasComments) {
  //       const confirmDelete = window.confirm(
  //         `This media has ${commentData.commentCount} related comment(s). ` +
  //           `Deleting this media will also permanently delete all related comments. ` +
  ...
  //     setMediaList((prev) => prev.filter((media) => media.mediaId !== mediaId));
  //     toast.success(
  //       commentData.hasComments
  //         ? `Media and ${commentData.commentCount} related comment(s) deleted successfully`
  //         : "Media deleted successfully",
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-4 items-center min-h-[400px] lg:min-h-[610px] ">
              <div className="order-2 lg:order-1 p-4 sm:px-8   space-y-4 lg:space-y-4">
                ...
                <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 pt-4 sm:pt-6 px-2">
                  <Link
                    href="/dashboard/instagram-automation/auto-dm-comment-link/automation/"
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <h2 className="text-lg font-semibold text-gray-900">
                    Auto-reply to comment in DM Automation Saved History
                  </h2>
                </div>
