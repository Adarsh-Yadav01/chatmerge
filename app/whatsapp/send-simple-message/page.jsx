'use client';

import { useState } from 'react';

export default function SendSimpleMessage() {
  const [phoneNumber, setPhoneNumber] = useState('+918840820604');
  const [messageText, setMessageText] = useState('Hello, this is a test message!');
  const [message, setMessage] = useState('');

  const handleSubmit = async (e) => {
    e.preventDefault();
    setMessage('');
    try {
      // Validate inputs
      if (!/^\+\d{10,15}$/.test(phoneNumber)) {
        setMessage('Error: Invalid phone number format (e.g., +918840820604)');
        return;
      }
      if (!messageText || messageText.length > 4096) {
        setMessage('Error: Message text is required and must be 4096 characters or less');
        return;
      }

      const response = await fetch('/api/whatsapp/send-simple-message', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ to: phoneNumber, text: messageText }),
      });
      const data = await response.json();
      if (response.ok) {
        setMessage(`Message sent successfully! ID: ${data.messages[0].id}`);
        setPhoneNumber('+918840820604');
        setMessageText('Hello, this is a test message!');
      } else {
        setMessage(`Error: ${data.error?.message || 'Failed to send message'}`);
      }
    } catch (error) {
      setMessage('Error: Could not connect to server or send message');
    }
  };

  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center py-10">
      <div className="w-full max-w-md bg-white p-6 rounded-xl shadow-lg">
        <h1 className="text-2xl font-semibold mb-4 text-center text-gray-800">Send Simple WhatsApp Message</h1>
        <p className="text-sm text-gray-600 mb-4 text-center">
          Send a test text message. Recipient must have opted in and messaged within 24 hours.
          Use a Meta sandbox number for testing.
        </p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Recipient Phone Number</label>
            <input
              type="text"
              value={phoneNumber}
              onChange={(e) => setPhoneNumber(e.target.value)}
              className="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
              placeholder="e.g., +918840820604"
              required
            />
            <p className="text-xs text-gray-500 mt-1">Use international format</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Message Text</label>
            <textarea
              value={messageText}
              onChange={(e) => setMessageText(e.target.value)}
              className="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
              placeholder="e.g., Hello, this is a test message!"
              rows={4}
              maxLength={4096}
              required
            />
            <p className="text-xs text-gray-500 mt-1">Up to 4096 characters</p>
          </div>
          <button
            type="submit"
            className="w-full bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700"
          >
            Send Test Message
          </button>
        </form>
        {message && (
          <p
            className={`mt-4 text-center text-sm ${
              message.includes('Error') ? 'text-red-600' : 'text-green-600'
            }`}
          >
            {message}
          </p>
        )}
      </div>
    </div>
  );
}