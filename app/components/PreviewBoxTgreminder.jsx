...
export default function PreviewBoxTgreminder({
  keywords,
  anyKeywordMode,
  description,
  greetingAttachment,
  ...
  const renderFilePreview = (attachment) => {
    ...
    if (attachment.type.startsWith("image/")) {
      return <img src={attachment.url} alt="attachment" className="w-full rounded-lg mb-2" />;
    } else if (attachment.type.startsWith("video/")) {
      return <video src={attachment.url} className="w-full rounded-lg mb-2" controls />;
    } else if (attachment.type.startsWith("audio/")) {
      return <audio src={attachment.url} className="w-full mb-2" controls />;
    } else {
      return (
        <div className="bg-gray-800 rounded-lg p-2 mb-2 flex items-center space-x-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <a href={attachment.url} download={attachment.name} className="text-xs text-blue-400 hover:underline truncate">
              Download
            </a>
          </div>
        </div>
                  ...
                  <div className="flex justify-start items-end space-x-1">
                    ...
                    <div className="max-w-[70%] space-y-2">
                      <div className="bg-white/20 rounded-2xl rounded-bl-xs px-4 py-2">
                        {renderFilePreview(greetingAttachment)}
                    ...
                    <>
                      ...
                      {showResponse && (
                        <div className="flex justify-start items-end space-x-1">
                          ...
                          <div className="max-w-[70%] space-y-2">
                            <div className="bg-white/20 rounded-2xl rounded-bl-xs px-4 py-2">
                              {renderFilePreview(buttons[selectedButtonIndex].imageAttachment)}
                              <p className="text-white text-xs mb-2 break-words">
